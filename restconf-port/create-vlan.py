...
def create_vlan(host, port, user, password, interface, vlan, ip, insecure):
	...
	print ("h=" + host + " p=" + port + " u=" + user + " pa=" + password + " i=" + interface + " v=" + str(vlan) + " ip=" + str(ip) + " in=" + str(insecure))
	...
	#print ("h=" + host + " p='" + port + "' mgroup1=" + str(m.group(1)) + " mgroup2=" + str(m.group(2)) + " v=" + str(vlan))	
	url       = "https://%s:%d/api/running/native/interface/%s/%s.%d" % (host, int(port), m.group(1), m.group(2), vlan)
	...
	try:
		...
		result = requests.put(url, auth=(user, password), data=data, headers=headers, verify=False)
	except:
